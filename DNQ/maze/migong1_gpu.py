...
env=gym.make('MazeWorld-v0')#吃豆人#MsPacman-ram-v0
MAX=1000
inf=0.001
INF=1000000
...
class DQN(object):#拟合Q的神经网络
    ...
    def learn(self):#学习以往经验
        ...
        sampleList=np.random.choice(MEMORY_CAPACITY-4, BATCH_SZ)#从[0,MEMORY_CAPACITY-1]区间随机选取BATCH_SZ个数
        ...
        state=torch.FloatTensor([self.memoryQue[0][sampleList],self.memoryQue[0][sampleList-1],self.memoryQue[0][sampleList-2],self.memoryQue[0][sampleList]])#state
    ...
    def save(self):
        torch.save(self.target_net, "../MsPacman/mod/target_net.pt")#保存模型pt || pth
        torch.save(self.eval_net, "../MsPacman/mod/eval_net.pt")  #
    def read(self):
...
def train(beginShape):
    ...
    for episode_i in range(EPOCHS_CNT):#循环若干次
        # print(episode_i)
        state = cv2.cvtColor(env.reset() , cv2.COLOR_BGR2GRAY)# 重新开始游戏,(210, 160, 3)
        is_terminal=False
